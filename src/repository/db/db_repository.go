...
import (
  ...
  "github.com/katsun0921/bookstore_oauth-api/src/clients/cassandra"
  "github.com/katsun0921/bookstore_oauth-api/src/domain/access_token"
...
func (r *dbRepository) GetById(id string) (*access_token.AccessToken, rest_errors.RestErr) {
	...
	if err := session.Query(queryGetAccessToken, id).Scan(
		...
		if err == gocql.ErrNotFound {
			return nil, rest_errors.NewNotFoundError("no access token found with given id", err)
		}
...
func (r *dbRepository) UpdateExpirationTime(at access_token.AccessToken) rest_errors.RestErr {
	...
	if err := session.Query(queryUpdateExpires,
		...
		return rest_errors.NewInternalServerError("error when tyring to update current resource",errors.New("database error"), )
	}
	return nil
}
